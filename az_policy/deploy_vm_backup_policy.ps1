# Define variables
$policyName = "Liberty Mutual VM Backup Policy"
$policyDescription = "Automatically configures backup for VMs tagged with 'backup-policy' to the appropriate Recovery Services Vault"
$policyDefinitionPath = ".\liberty_mutual_vm_backup_policy.json"
...
# Always create a new policy definition to ensure we have the latest version
Write-Host "Creating or updating policy definition '$policyName'..."
try {
    # Read the policy definition from the file
    $policyContent = Get-Content -Path $policyDefinitionPath -Raw
    
    # Create a new policy definition or update existing one
    $existingPolicy = Get-AzPolicyDefinition | Where-Object { $_.Properties.DisplayName -eq $policyName }
    
    if ($existingPolicy) {
        Write-Host "Found existing policy. Updating..."
        $definition = Set-AzPolicyDefinition -Name $policyName -DisplayName $policyName -Description $policyDescription -Policy $policyContent -Mode Indexed
    } else {
    ...
    Write-Host "Policy definition object returned:"
...
# Check if specific backup policies exist in both vaults
...
$assignment = New-AzPolicyAssignment -Name $assignmentName `
    -DisplayName "Liberty Mutual VM Backup Policy" `
...
$attempt = 0
$principalId = $null
while ($attempt -lt $maxAttempts -and (-not $assignment.Identity -or -not $assignment.Identity.PrincipalId)) {
